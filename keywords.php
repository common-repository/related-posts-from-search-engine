...
function drpp_extract_keywords($source,$max = 20) {
...
function post_title_keywords($ID,$max = 20) {
	global $wpdb;
	return drpp_extract_keywords(html_entity_strip($wpdb->get_var("select post_title from $wpdb->posts where ID = $ID")),$max);
}
...
function post_body_keywords($ID,$max = 20) {
	...
	return drpp_extract_keywords($content,$max);
}

function drpp_cache_keywords($ID) {
	global $wpdb, $drpp_debug;
    $body_terms = post_body_keywords($ID);
    $title_terms = post_title_keywords($ID);
	/*
	CREATE TABLE `wp_drpp_keyword_cache` (
	`ID` BIGINT( 20 ) UNSIGNED NOT NULL ,
	`body` TEXT NOT NULL ,
	`title` TEXT NOT NULL ,
	`date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ,
	PRIMARY KEY ( `ID` )
	) ENGINE = MYISAM COMMENT = 'drpp\'s keyword cache table' 
	*/
	...
	$wpdb->query("insert into {$wpdb->prefix}drpp_keyword_cache (ID,body,title) values ($ID,'$body_terms ','$title_terms ') on duplicate key update date = now(), body = '$body_terms ', title = '$title_terms '");
	
}

function drpp_get_cached_keywords($ID,$type='body') {
	global $wpdb;
	$out = $wpdb->get_var("select $type from {$wpdb->prefix}drpp_keyword_cache where ID = $ID");
	if ($out === false or $out == '') { // if empty, try caching them first.
		drpp_cache_keywords($ID);
    $out = $wpdb->get_var("select $type from {$wpdb->prefix}drpp_keyword_cache where ID = $ID");
  }
	if ($out === false or $out == '') { // if still empty... return false
		//echo "<!--drpp ERROR: couldn't select/create drpp $type keywords for $ID-->";
    return false;
	} else {
		return $out;
  }
