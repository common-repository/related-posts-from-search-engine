...
if ($related_query->have_posts()) {
	while ($related_query->have_posts()) {
		...
		if ($userdata->user_level >= 8 and $domain != 'rss')
			$output .= ' <abbr title="'.sprintf(__('%f is the drpp match score between the current entry and this related entry. You are seeing this value because you are logged in to WordPress as an administrator. It is not shown to regular visitors.','drpp'),round(get_the_score(),3)).'">('.round(get_the_score(),3).')</abbr>';
		$output .= '</a>';
		if ($show_excerpt) {
			$output .= $before_post . 
			  drpp_excerpt(get_the_excerpt(),$excerpt_length)
			  . $after_post;
		}
		$output .=  $after_title."\n";
